#!/usr/bin/env python3
"""Generate a 1024x1024 sprite sheet template
    with 4x4 grid, numbered circles."""
...
def _colors_clash(a: str, b: str, threshold: int = 60) -> bool:
    ...
    return all(
        abs(int(a[i:i + 2], 16) - int(b[i:i + 2], 16)) < threshold
        for i in (0, 2, 4)
    )
...
def make_template(
    ...
    if _colors_clash(bg, line_color):
        line_color = "#0000ff"
        assert not _colors_clash(
            bg, line_color
        ), f"BG {bg} clashes with both red and blue line colors"
    ...
    # Border
    draw.rectangle(
        [0, 0, SIZE - 1, SIZE - 1], outline=line_color, width=LINE_W
    )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "-o", "--output", default="spritesheet_template.png"
    )
    p.add_argument("--bg", default="#1a1a1a", help="Background color")
    p.add_argument(
        "--line-color", default="#ff0000", help="Grid line color"
    )
    p.add_argument(
        "--circle-color", default="#ffffff", help="Circle fill color"
    )
    p.add_argument(
        "--text-color", default="#000000", help="Number text color"
    )
    args = p.parse_args()
    make_template(
        args.output, args.bg, args.line_color,
        args.circle_color, args.text_color
    )
