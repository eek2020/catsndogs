...
Terrain types produced:
  ...
  - Decor row (row 6): hedge, tree_trunk, tree_top, flower, barrel, crate, lamp, sign, door
  - Extra row (row 7): door_top, window, stairs_up, stairs_down, chest, table, chair, bed, rug
...
# 3x3 autotile positions: top-left, top, top-right, left, center, right, bottom-left, bottom, bottom-right
...
def _darken(rgb: tuple[int, int, int], factor: float = 0.7) -> tuple[int, int, int]:
...
def _draw_terrain_tile(draw: ImageDraw.ImageDraw, x: int, y: int, base_rgb: tuple, label: str, font) -> None:
    """Draw a single terrain autotile cell with visual variation."""
    # Fill with slight variation based on autotile position
    idx = AUTOTILE_LABELS.index(label) if label in AUTOTILE_LABELS else 4
    ...
    if "T" in label and label != "TR":
        draw.line([(x, y), (x + TILE - 1, y)], fill=edge_color, width=2)
    if "B" in label and label != "BR":
        draw.line([(x, y + TILE - 1), (x + TILE - 1, y + TILE - 1)], fill=edge_color, width=2)
    if "L" in label and label != "BL":
        draw.line([(x, y), (x, y + TILE - 1)], fill=edge_color, width=2)
    if "R" in label and label != "TR":
        draw.line([(x + TILE - 1, y), (x + TILE - 1, y + TILE - 1)], fill=edge_color, width=2)
...
def _draw_decor_tile(draw: ImageDraw.ImageDraw, x: int, y: int, hex_color: str, label: str, font) -> None:
    """Draw a single decor/extra tile."""
    rgb = _hex_to_rgb(hex_color)
    # Background
    draw.rectangle([x, y, x + TILE - 1, y + TILE - 1], fill=rgb)
    # Border
    draw.rectangle([x, y, x + TILE - 1, y + TILE - 1], outline=_darken(rgb, 0.5), width=1)
    # Centered label
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Tile atlas generator for Godot TileMap")
