...
try:
    ...
    # We will compute sum of absolute differences to find where
    # Frame 1, 2, 3 are relative to Frame 0
    # Let's assume Frame 0 is centered around X=211.
    # Let's take [160, 260] as the template string.
    ...
    def get_template(x_start):
        ...
        for x in range(x_start, x_start + 100):
            ...
            for y in range(Y, Y + 120):
                r, g, b, a = p[x, y]
                col.append(
                    1 if a > 10 and (r, g, b, a) not in bg_colors else 0
                )
            mask.append(col)
        return mask
    ...
    for frame_idx in range(1, 4):
        ...
        for x in range(search_start, search_end):
            ...
            if diff < best_diff:
                ...
                best_x = x
        print(
            f"Frame {frame_idx} best match relative to x=160 "
            f"happens at x={best_x}. Distance: {best_x - 160}"
        )
