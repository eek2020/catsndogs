...
**Script section ordering:** signals -> @onready vars -> @export vars -> private state -> lifecycle methods -> public methods -> private methods -> signal handlers
...
- **CRITICAL: NEVER use `:=` with polymorphic math functions** — `abs`, `sign`, `clamp`, `min`, `max`, `floor`, `ceil`, `round`, `lerp`, `smoothstep`, `move_toward`, `wrap`, `snappedf`, `randf_range`, `randi_range` return Variant. Use explicit types: `var x: float = abs(y)` not `var x := abs(y)`

## VehicleBody3D Example
