...
Screenshot and video capture workflows for Godot projects. Supports GPU-accelerated rendering with software fallback.

## GPU Detection

Run once per session to detect available GPU:
```bash
GPU_DISPLAY=""
...
**Requires GPU** — video capture only works with a GPU display. Software rendering is too slow for video.
...
**AVI to MP4:** Godot outputs MJPEG AVI. ffmpeg converts to H.264 MP4. CRF 28 + `-preset slow` targets ~2-5MB for a 30s clip at 720p.

## Mac-Specific Notes

On macOS (the primary target for Whisper Crystals), GPU detection via X11 is not applicable. Use Godot's native rendering directly:

```bash
timeout 30 godot --write-movie screenshots/{task}/frame.png \
    --fixed-fps 10 --quit-after {N} \
    --script test/test_task.gd 2>&1
```
...
Always create `screenshots/.gdignore` to prevent Godot's resource importer from processing captured images:
```bash
