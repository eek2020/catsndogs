...
5. Attaches scripts via `set_script()`
...
**GLB orientation:** Imported models often face the wrong axis. After instantiating, check the AABB: the longest dimension tells you which local axis the model faces. Verify in screenshots.

**Collision shapes for 3D models:** Always use simple primitives (BoxShape3D, SphereShape3D, CapsuleShape3D). Never use `create_convex_shape()` or `create_trimesh_shape()` on imported GLB meshes — causes <1 FPS on high-poly models.
...
- Use ONLY nodes and resources available in Godot — look up unfamiliar classes in `api/` docs
...
- ALWAYS set `.name` on every node you create — scripts need predictable names for `@onready` references
...
- **2D/3D consistency** — Use ONLY 2D nodes OR 3D nodes. Never mix dimensions in the same scene hierarchy.
