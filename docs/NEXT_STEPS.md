...
| 4 | Props + NPCs: 14 tree billboards (serene_village sheet), 2 procedural 3D campfires (crossed logs + emissive cone + flickering OmniLight), 3 NPCs using full `Character3D` rigs (new `felid_corsair_guard` — Fringe Haven is felid-corsair territory) each wrapped in a StaticBody3D capsule so the player bumps into them. Roof-lift formula recomputed (`h/2·cos30° + d/2·sin30°` + 0.15m margin) and doghouse spacing widened to 2.6m so adjacent roofs no longer z-fight. Path crossroads offset (N-S y=0.010, E-W y=0.012) removes centre-tile jitter. | **Done 2026-04-20** |
| 5 | Interactions: Area3D zones per interactable feed `_active_zones` on `body_entered`/`body_exited`; `interact` action dispatches via `_try_interact`. Bryn opens `GameSession.open_trade_screen("felid_corsairs")` + `main.push_overlay("trade")`. Hidden chest (wooden box + golden band + emissive sparkle + `StaticBody3D` box collider so the player can't walk through) grants +25 crystals / +10 salvage once, persists via `story_flags["fringe_haven_chest_opened"]`, lid tilts on collect. CanvasLayer HUD: fixed "DEPART — ESC" hint, proximity-gated "[E] Interact" hint, centred flash message line. Depart skips `planet_system.depart()` (hub isn't reached via `land_on`) and calls `MusicManager.on_state_change("navigation")` + `main.switch_scene("navigation")`. | **Done 2026-04-20** |
| 6 | Trader Bryn + shop interior + input polish: `trader_bryn` FBX rig registered in `Character3D.CHARACTER_BASES`; new `_make_shop_building` carves a 1.2m doorway, replaces pitched roof with an awning, adds counter + lamp + plank floor. Bryn stands at (6,0,-5.4) with an emissive vendor spot under her. Area3D triggers swapped for a per-tick XZ distance poll (`_poll_interact_zones`) after Area3D+`body_entered` failed on both chest and Bryn. New `sprint` action (Shift / L3) with `SPRINT_SPEED=4.5` vs `MOVE_SPEED=1.8`; `CAMERA_YAW_DEG` 30→0 so WASD aligns with screen axes. HUD shows RUN hint. | **Done 2026-04-20** |
| 7 | Scene-registry swap: `navigation.gd:FRINGE_HAVEN_SCENE_PATH` + `star_map_screen.gd:WORLD_SCENE_MAP["starting_realm"]` now point at `fringe_haven_3d.tscn`. Old 2D outpost scenes kept as design reference for future 3D planet builds. Tavern ExitDoor parked (3D flow never loads tavern). Oakhaven + procedural `planet_surface` ports still pending. | **Done 2026-04-21** (Fringe Haven only) |
| 8 | Bryn Mixamo anims: idle/walk/run/jump/laugh FBX landed at `godot/assets/characters/npc/trader_bryn/3d/animations/` (fixed `_anim__walk` → `_anim_walk` typo). Rig now animates in-scene via `Character3D`. | **Done 2026-04-21** |
| 9 | Bryn's shop 3D interior: `_make_shop_building` now dresses the interior with side-wall shelves (stocked jars/bottles/crates), counter wares, back-corner overflow crates, stool behind counter, doorway rug, and a second ceiling lamp for even interior lighting. Seamless walk-in — no scene transition needed; open-top roof preserves 3/4 camera readability. | **Done 2026-04-21** |
| 10 | Procedural `planet_surface` 3D port: new `scripts/world/planet_surface_3d.gd` + `scenes/world/planet_surface_3d.tscn`. `main.gd` SCENES["planet"] now routes here. Seeded-on-planet_id procedural layout (ground + crossroads + 4–8 buildings + ring of trees), merchants as `felid_corsair_guard` rigs w/ real faction_id trade, treasures via `planet_system.collect_treasure`/`is_treasure_cleared` (chest lid tilts on collect), `planet_system.depart(gs)` flushes inventory on ESC. HUD: planet name + loot counter + depart/run/interact hints + flash. Oakhaven never routed through main's planet path (2D `oakhaven_outpost.tscn` stays as design reference only). | **Done 2026-04-21** |
| 11 | Bryn shop face-on dialogue + trade: new `scenes/world/bryn_shop_interior.tscn` + `scripts/world/bryn_shop_interior.gd`. Node3D-rooted (Control + SubViewportContainer caused skinned-mesh rig to render invisible — see step 11b). Close-up ortho camera (distance 3, ortho_size 2.6) on Bryn; back-wall shelves w/ small jars/crates framing her upper body; compact 112px bottom dialogue strip with first-visit intro gated by `story_flags["bryn_shop_first_visit"]`. `TRADE` instantiates `trade_screen.tscn` as a CanvasLayer child (local `pop_overlay` routes close back to dialogue); `LEAVE`/ESC `change_scene_to_file`s back to `fringe_haven_3d` with player spawned at doorway via new `GameSession.pending_fringe_haven_spawn`. Also fixed a latent bug: Bryn's trade overlay in the outdoor scene never worked (pushing overlay on a scene with no main.gd hierarchy) — the new flow is the first working Bryn-trade path. | **Done 2026-04-21** |
| 12 | DualSense / Bluetooth gamepad input: `project.godot` input map has no joypad events bound — `interact`, `sprint`, `pause`, movement are keyboard-only. Audit actions, add `InputEventJoypadButton` / `InputEventJoypadMotion` entries for the full set. Also decide mapping (e.g. sprint = L3 stick press or L2 analogue trigger). | **Parked 2026-04-21** — deferred until much later; keyboard-only is acceptable for development. Re-open when controller support becomes a release blocker or when playtesting demands it. |
| 13 | Smoke-test `planet_surface_3d.tscn` end-to-end. It was shipped in step 10 using the Control + SubViewportContainer pattern that was later found to cause Character3D rigs to render invisible at close ortho scales (see CHANGELOG step 11b). A merchant landing on a procedural planet may hit the same bug. Either verify it renders the `felid_corsair_guard` rig correctly, or port to Node3D root with CanvasLayer HUD (same fix as bryn_shop). | **Done 2026-04-21** — visual verification (player + two `felid_corsair_guard` merchants render correctly at ortho_size 6 / distance 18) + new GUT regression `tests/unit/test_planet_surface_3d.gd` (4/4 pass): scene mounts SubViewport tree, merchants mount as Character3D rigs, treasure round-trips `collect_treasure`/`is_treasure_cleared`, depart flushes `planet_inventory` into ship. Full suite 256/256 green. Incidental fix: `data/characters/protagonists.json` had an invalid `\'` escape on line 120 that was silently failing `load_protagonists` (returning `{}`) — caught because the test exercised `start_new_game`. SubViewport pattern is empirically fine at this camera range; don't reuse it for close-up scenes. |
| 14 | Shop interior walls outside frame: with the close ortho camera in Bryn's shop, the side + back walls are no longer in frame. Current composition works because the warm backdrop reads as an interior, but if the shot ever widens the walls need reconsidering — consider painted backdrop plane or clearer framing elements. | **Done 2026-04-21** (step 11c: painted backdrop swap) |
| 15 | Painted-backdrop + 3D rig pattern landed in step 11c: `bryn_shop_interior` now uses a single `QuadMesh` with the painted PNG instead of procedural walls/shelves/jars, plus `Character3D.model_scale = 2.1` locally to match the painting's human-scale shelves. Candidate scenes for the same pattern: other NPC shops/dialogue rooms that don't need interactive props. Avoid for walkable spaces (camera needs to pan/rotate). | Pending (as-needed) |
| 16 | Fringe Haven exterior material pass: 6 CC0 Poly Haven seamless JPGs (1K) shipped to `godot/assets/textures/fringe_haven/` (grass, cobblestone, mud, plaster, wood planks, roof tiles) with attribution in `CREDITS.txt`. `_build_ground` → seamless grass at 4m per repeat (linear + mipmaps, replacing NEAREST 16px). `_add_path_strip` signature changed from `tile: Vector2i` to `texture_path: String`; cobblestone for the stone crossroads, mud for the dirt branches, 2.5m per repeat. `_make_building` walls gain plaster tex + triplanar (albedo_color keeps the warm-plaster tint), roofs gain roof-tile tex (per-building roof_color survives as tint), doors gain wood-plank tex. `_make_shop_building` walls + awning follow the same pattern. Pixel-art tileset PNG stays in use for tree/campfire billboards. | **Done 2026-04-21** |
...
- **3D ships — live wiring (from 2026-04-21).** `felid_cruiser.fbx` (500k tris, first faction-authored 3D ship) smoke-tested in `ship_3d_preview.tscn`; renders cleanly at 512² SubViewport. Next steps: (a) author 3D models for the other 7 factions (league_cruiser, wolf_ship, goblin_scrapper, fairy_ship, royal_galleon, knight_ship, alien_vessel, league_destroyer) so the same pattern can replace all 2D ship PNGs; (b) wire the SubViewport ship render into `navigation.gd`'s ship sprite; (c) decimate *only* when build size crosses 100 MB or when combat screens composite 3+ ships per frame.
