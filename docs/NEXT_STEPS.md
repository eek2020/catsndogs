...
1. **Sprint 6c** — multi-slot saves + tutorial encounter + game feel polish. The remaining 6-series slice.
2. **Star map connectivity polish** — `_request_travel` shows a confirm dialog for any discovered region; travel only succeeds for directly connected ones, so non-connected targets fail silently. Gate the dialog on connectivity + surface a "not connected — route via X" message. Small (1–2 h).
...
5. **Nav `_draw` decomposition** — `navigation.gd` is still 1,700+ lines with pre-existing lint warnings (long lines, private-access, stale Python-docstring shards). The recent fog tweak and nebula removal left the file messier, not cleaner. Candidate for a 3b-style extraction into `scripts/ui/navigation/` helpers. Medium (half-day).
