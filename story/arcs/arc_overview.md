# Whisper Crystals — Story Arc Overview

## Arc Structure

The narrative spans 4 arcs, each escalating in scope, complexity, and stakes. Arcs are sequential — each must be completed before the next unlocks.
...
**Decision Point:** Player chooses initial stance — aggressive expansion, cautious trade, or isolationist defence

**Exit Conditions:** `arc1_crystal_discovered`, `arc1_dave_met`, `arc1_death_glimpsed` all true
...
- Multiverse faction offers alliance with conditions

**Decision Point:** Player chooses primary front — external (Dave), internal (Death), or political (Lions)

**Exit Conditions:** `arc2_route_resolved`, `arc2_death_betrayal`, `arc2_lion_response` all set
...
**Decision Point:** Player builds alliance portfolio — which factions to ally with and oppose

**Exit Conditions:** `arc3_alien_contact`, `arc3_dave_parley`, `arc3_death_allegiance` all set
